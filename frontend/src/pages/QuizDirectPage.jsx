...
function QuizScreen({ quiz, totalSeconds, onFinish }) {
    ...
    const finish = useCallback((expired = false) => {
        ...
        questions.forEach(question => {
            ...
            if (correctChoice && chosenId === correctChoice.id) score++;
        });
        onFinish({ score, total: questions.length, percent: Math.round((score / questions.length) * 100), timeExpired: expired });
        ...
        <div style={{ minHeight: "100vh", background: "#F9FAFB", fontFamily: "Inter, system-ui, sans-serif", display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
            <div style={{ width: "100%", maxWidth: 480 }}>
                <div style={{ background: "#fff", border: "1px solid #E5E7EB", borderRadius: 12, padding: "32px 28px", textAlign: "center", marginBottom: 16 }}>
                    <div style={{ width: 64, height: 64, borderRadius: "50%", margin: "0 auto 16px", background: "#DBEAFE", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 28 }}>
                        🎯
                    </div>
                    <h1 style={{ margin: "0 0 4px", fontSize: 22, fontWeight: 700, color: "#111827" }}>Quiz terminé !</h1>
                    <p style={{ margin: "0 0 24px", fontSize: 13, color: "#6B7280" }}>{quiz.title}</p>

                    <div style={{ position: "relative", display: "inline-flex", alignItems: "center", justifyContent: "center" }}>
                        <svg width="120" height="120" style={{ transform: "rotate(-90deg)" }}>
                            <circle cx="60" cy="60" r="50" fill="none" stroke="#F3F4F6" strokeWidth="8" />
                            <circle cx="60" cy="60" r="50" fill="none" stroke="#4F6FE8" strokeWidth="8"
                ...
                <div style={{ background: "#fff", border: "1px solid #E5E7EB", borderRadius: 12, padding: "20px 24px", marginBottom: 16 }}>
                    {[
                        { label: "Bonnes réponses", value: result.score, color: "#059669" },
                        { label: "Mauvaises réponses", value: result.total - result.score, color: "#EF4444" },
                    ].map((row, i) => (
...
export default function QuizDirectPage() {
    const { id } = useParams();
    ...
    useEffect(() => {
        axios.get(`${API_BASE}/api/quiz/direct/${id}`)
            .then(res => { setQuiz(res.data); setPhase("quiz"); })
            .catch(() => { setError("Quiz introuvable ou non publié."); setPhase("error"); });
    }, [id]);

    const handleFinish = (resultData) => { setResult(resultData); setPhase("results"); };
    const totalSeconds = quiz?.timerSeconds ?? 0;
